...
cmd="python ~/hetdex/hetdex_cube/detection_noise_model.py -i {filename} -o {outfilename}"

slurmtempl="run_detectnoisemodel.slurm"


parser = argparse.ArgumentParser()
parser.add_argument('-N', '--njobs', default=24, type=int)
parser.add_argument('-s', '--slurms_dir',  default="slurms_detectnoisemodel", type=str)
...
worklist = glob("data/sf2outcube*.fits.gz")
...
for j,ii in enumerate(iii):
    ...
    with open(path, 'w') as f:
        for i in ii:
            ...
            outfilename  = os.path.join(h,t).replace(".fits.gz", ".detect_noise_model")
